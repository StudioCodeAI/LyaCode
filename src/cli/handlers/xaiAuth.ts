/**
 * `lyacloud auth xai ...` command handlers.
 *
...
function listenForManualCode(onLine: (line: string) => void): () => void {
  ...
  // (`lyacloud auth xai login`) stays alive after a successful
...
export async function xaiLogout(deps?: XaiLogoutDeps): Promise<void> {
  ...
  // xAI OAuth interactively and then ran `lyacloud auth xai logout`
  // ends up in a clean state. Without all four steps, the
  // marker-tagged .lyacloud-profile.json survives, startup
