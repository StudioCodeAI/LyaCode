/**
 * `lyacode auth xai ...` command handlers.
 *
...
function listenForManualCode(onLine: (line: string) => void): () => void {
  ...
  // (`lyacode auth xai login`) stays alive after a successful
...
export async function xaiLogout(deps?: XaiLogoutDeps): Promise<void> {
  ...
  // xAI OAuth interactively and then ran `lyacode auth xai logout`
  // ends up in a clean state. Without all four steps, the
  // marker-tagged .lyacode-profile.json survives, startup
