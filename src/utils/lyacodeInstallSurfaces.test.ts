...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/lyacodeInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/lyacode on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/lyacode')
})

test('install command displays lyacode.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'lyacode.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both lyacode and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.lyacode'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.lyacode', 'local'))
