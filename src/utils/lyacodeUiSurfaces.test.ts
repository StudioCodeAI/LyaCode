...
const originalLyaCodeConfigDir = process.env.LYACODE_CONFIG_DIR

beforeEach(async () => {
  await acquireSharedMutationLock('lyacodeUiSurfaces.test.ts')
  mock.restore()
...
afterEach(() => {
  try {
    ...
    if (originalLyaCodeConfigDir === undefined) {
      delete process.env.LYACODE_CONFIG_DIR
    } else {
      process.env.LYACODE_CONFIG_DIR = originalLyaCodeConfigDir
    }
  } finally {
    releaseSharedMutationLock()
  }
...
describe('Lya Code settings path surfaces', () => {
  test('isClaudeSettingsPath recognizes project .lyacode settings files', () => {
    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.lyacode', 'settings.json'),
      ),
    ).toBe(true)

    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.lyacode', 'settings.local.json'),
      ),
    ).toBe(true)
  })

  test('permission save destinations point user settings to configured LYACODE_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-lyacode')
    process.env.LYACODE_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('skills help surfaces point user skills to configured LYACODE_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-lyacode')
    process.env.LYACODE_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('permission save destinations point project settings to .lyacode', async () => {
    const { optionForPermissionSaveDestination } = await import(
      '../components/permissions/rules/AddPermissionRules.tsx'
    )
    ...
    expect(optionForPermissionSaveDestination('projectSettings')).toEqual({
      ...
      description: 'Checked in at .lyacode/settings.json',
      value: 'projectSettings',
    })

    expect(optionForPermissionSaveDestination('localSettings')).toEqual({
      label: 'Project settings (local)',
      description: 'Saved in .lyacode/settings.local.json',
      value: 'localSettings',
    })
  })

  test('permission dialog treats ~/.lyacode as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.lyacode')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), '.lyacode', 'settings.json'),
      ),
    ).toBe(true)
    expect(
  ...
  test('permission dialog does not treat arbitrary CLAUDE_CONFIG_DIR as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), 'custom-lyacode')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), 'custom-lyacode', 'settings.json'),
      ),
    ).toBe(false)
  })

  test('global skill scope recognizes ~/.lyacode and legacy ~/.claude skills', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.lyacode')

    expect(
      getClaudeSkillScope(
        join(homedir(), '.lyacode', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toEqual({
      skillName: 'demo',
      pattern: '~/.lyacode/skills/demo/**',
    })
  ...
  test('global skill scope does not emit fixed rules for arbitrary CLAUDE_CONFIG_DIR skills', () => {
    ...
    expect(
      getClaudeSkillScope(
        join(homedir(), 'custom-lyacode', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toBe(null)
  })
})

describe('Lya Code validation tips', () => {
...
describe('Lya Code permission mode surfaces', () => {
