...
const originalLyaCloudConfigDir = process.env.LYACLOUD_CONFIG_DIR

beforeEach(async () => {
  await acquireSharedMutationLock('lyacloudUiSurfaces.test.ts')
  mock.restore()
...
afterEach(() => {
  try {
    ...
    if (originalLyaCloudConfigDir === undefined) {
      delete process.env.LYACLOUD_CONFIG_DIR
    } else {
      process.env.LYACLOUD_CONFIG_DIR = originalLyaCloudConfigDir
    }
  } finally {
    releaseSharedMutationLock()
  }
...
describe('Lya Cloud settings path surfaces', () => {
  test('isClaudeSettingsPath recognizes project .lyacloud settings files', () => {
    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.lyacloud', 'settings.json'),
      ),
    ).toBe(true)

    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.lyacloud', 'settings.local.json'),
      ),
    ).toBe(true)
  })

  test('permission save destinations point user settings to configured LYACLOUD_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-lyacloud')
    process.env.LYACLOUD_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('skills help surfaces point user skills to configured LYACLOUD_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-lyacloud')
    process.env.LYACLOUD_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('permission save destinations point project settings to .lyacloud', async () => {
    const { optionForPermissionSaveDestination } = await import(
      '../components/permissions/rules/AddPermissionRules.tsx'
    )
    ...
    expect(optionForPermissionSaveDestination('projectSettings')).toEqual({
      ...
      description: 'Checked in at .lyacloud/settings.json',
      value: 'projectSettings',
    })

    expect(optionForPermissionSaveDestination('localSettings')).toEqual({
      label: 'Project settings (local)',
      description: 'Saved in .lyacloud/settings.local.json',
      value: 'localSettings',
    })
  })

  test('permission dialog treats ~/.lyacloud as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.lyacloud')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), '.lyacloud', 'settings.json'),
      ),
    ).toBe(true)
    expect(
  ...
  test('permission dialog does not treat arbitrary CLAUDE_CONFIG_DIR as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), 'custom-lyacloud')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), 'custom-lyacloud', 'settings.json'),
      ),
    ).toBe(false)
  })

  test('global skill scope recognizes ~/.lyacloud and legacy ~/.claude skills', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.lyacloud')

    expect(
      getClaudeSkillScope(
        join(homedir(), '.lyacloud', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toEqual({
      skillName: 'demo',
      pattern: '~/.lyacloud/skills/demo/**',
    })
  ...
  test('global skill scope does not emit fixed rules for arbitrary CLAUDE_CONFIG_DIR skills', () => {
    ...
    expect(
      getClaudeSkillScope(
        join(homedir(), 'custom-lyacloud', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toBe(null)
  })
})

describe('Lya Cloud validation tips', () => {
...
describe('Lya Cloud permission mode surfaces', () => {
