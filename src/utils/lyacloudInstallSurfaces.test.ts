...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/lyacloudInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/lyacloud on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/lyacloud')
})

test('install command displays lyacloud.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'lyacloud.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both lyacloud and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.lyacloud'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.lyacloud', 'local'))
