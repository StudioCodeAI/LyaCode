...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'lyacloud-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'lyacloud', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'lyacloud')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'lyacloud', { status: 'alpha' })
    await addGlobalEntity('tool', 'lyacloud', { status: 'beta', version: '0.6.0' })
