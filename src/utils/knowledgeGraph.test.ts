...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'lyacode-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'lyacode', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'lyacode')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'lyacode', { status: 'alpha' })
    await addGlobalEntity('tool', 'lyacode', { status: 'beta', version: '0.6.0' })
