...
const BIN_PATH = join(import.meta.dir, '..', 'bin', 'lyacode')

describe('lyacode launcher heap guard', () => {
  ...
  test('keeps user and troubleshooting escape hatches', () => {
    ...
    expect(source).toContain('LYACODE_DISABLE_HEAP_RELAUNCH')
    expect(source).toContain('LYACODE_NODE_MAX_OLD_SPACE_SIZE_MB')
