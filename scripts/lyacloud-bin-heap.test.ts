...
const BIN_PATH = join(import.meta.dir, '..', 'bin', 'lyacloud')

describe('lyacloud launcher heap guard', () => {
  ...
  test('keeps user and troubleshooting escape hatches', () => {
    ...
    expect(source).toContain('LYACLOUD_DISABLE_HEAP_RELAUNCH')
    expect(source).toContain('LYACLOUD_NODE_MAX_OLD_SPACE_SIZE_MB')
