# Lya Cloud on Android (Termux)

A complete guide to running Lya Cloud on Android using Termux + proot Ubuntu.
...
Lya Cloud requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build Lya Cloud

```bash
cd /data/data/com.termux/files/home/lyacloud
bun run build
```

You should see:
```
✓ Built lyacloud v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to Lya Cloud's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Lya Cloud repo.
