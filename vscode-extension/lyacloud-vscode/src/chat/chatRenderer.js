...
<head>
  ...
  <style>
    :root {
      ...
      --lc-critical: #ff8a6c;
      --lc-focus: #ffd3a1;
      --lc-user-bg: rgba(240,148,100,0.12);
      --lc-user-border: rgba(240,148,100,0.28);
      --lc-assistant-bg: rgba(255,255,255,0.03);
      --lc-assistant-border: rgba(220,195,170,0.10);
      --lc-code-bg: #1a1310;
      --lc-code-border: rgba(220,195,170,0.12);
      --lc-tool-bg: rgba(232,184,107,0.06);
      --lc-tool-border: rgba(232,184,107,0.22);
      --lc-perm-bg: rgba(255,138,108,0.08);
      --lc-perm-border: rgba(255,138,108,0.35);
    }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chat-header .brand-accent { color: var(--lc-accent-bright); }
    .header-btn {
    ...
    .header-btn:hover { border-color: var(--lc-accent); color: var(--lc-text); }
    .header-btn.danger { border-color: var(--lc-critical); color: var(--lc-critical); }
    ...
    .status-bar .status-dot.connected { background: var(--lc-positive); }
    .status-bar .status-dot.streaming { background: var(--lc-accent-bright); animation: pulse 1s infinite; }
    .status-bar .status-dot.error { background: var(--lc-critical); }
    @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.4; } }
    .status-text { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .status-usage { color: var(--lc-text-soft); }
    ...
    .welcome-title { font-size: 20px; font-weight: 700; color: var(--lc-text); }
    .welcome-title .accent { color: var(--lc-accent-bright); }
    .welcome-sub { font-size: 13px; color: var(--lc-text-dim); max-width: 36ch; }
    .welcome-hint { font-size: 11px; color: var(--lc-text-soft); }
    .welcome-hint kbd {
      padding: 2px 6px;
      border-radius: 4px;
      border: 1px solid var(--lc-border-soft);
    ...
    .msg-user {
      ...
      border: 1px solid var(--lc-user-border);
    ...
    .msg-assistant {
      ...
      border: 1px solid var(--lc-assistant-border);
      word-break: break-word;
    }
    ...
    .msg-assistant .md-content a { color: var(--lc-accent-bright); text-decoration: underline; }
    .msg-assistant .md-content strong { color: var(--lc-text); font-weight: 700; }
    .msg-assistant .md-content em { font-style: italic; color: var(--lc-text-dim); }
    ...
    .code-header {
      ...
      font-size: 11px;
      color: var(--lc-text-soft);
      border-bottom: 1px solid var(--lc-code-border);
    ...
    .code-copy-btn:hover { background: rgba(255,255,255,0.08); color: var(--lc-text); }
    .code-block {
      display: block;
      padding: 10px 12px;
      overflow-x: auto;
    ...
    .tool-card {
      margin: 8px 0;
      border-radius: 8px;
      border: 1px solid var(--lc-tool-border);
      background: var(--lc-tool-bg);
      overflow: hidden;
    }
    .tool-header {
      display: flex;
    ...
    .tool-name { font-weight: 600; font-size: 12px; color: var(--lc-text); flex: 1; }
    .tool-status { font-size: 11px; color: var(--lc-text-soft); }
    .tool-status.running { color: var(--lc-accent-bright); }
    .tool-status.error { color: var(--lc-critical); }
    .tool-status.complete { color: var(--lc-positive); }
    .tool-chevron {
      font-size: 10px;
      color: var(--lc-text-soft);
    ...
    .tool-body {
      ...
      padding: 0 10px 10px;
      font-size: 12px;
      border-top: 1px solid var(--lc-tool-border);
    }
    ...
    .tool-output-content.error { color: var(--lc-critical); }
    .tool-path {
      font-weight: 400;
      color: var(--lc-text-soft);
      font-size: 11px;
    ...
    .file-link {
      ...
      border-bottom: 1px dotted var(--lc-accent);
      transition: color 120ms, border-color 120ms;
    }
    .file-link:hover {
      color: var(--lc-focus);
      border-bottom-color: var(--lc-focus);
    }
    .tool-input-content.tool-diff-old {
      border-left: 3px solid var(--lc-critical);
      padding-left: 10px;
      color: #ff9e8a;
    ...
    .tool-input-content.tool-diff-new {
      border-left: 3px solid var(--lc-positive);
      padding-left: 10px;
      color: #c8e6a0;
    }
    .tool-diff-btn {
    ...
    .perm-card {
      ...
      padding: 10px 12px;
      border-radius: 8px;
      border: 1px solid var(--lc-perm-border);
      background: var(--lc-perm-bg);
    }
    .perm-title { font-weight: 700; font-size: 12px; color: var(--lc-critical); margin-bottom: 6px; }
    .perm-desc { font-size: 12px; color: var(--lc-text-dim); margin-bottom: 8px; }
    .perm-input {
      padding: 6px 8px;
      margin-bottom: 8px;
      border-radius: 6px;
    ...
    .perm-btn.allow {
      ...
      border-color: var(--lc-positive);
      color: var(--lc-positive);
    }
    .perm-btn.deny {
      background: rgba(255,138,108,0.1);
      border-color: var(--lc-critical);
    ...
    .input-area textarea::placeholder { color: var(--lc-text-soft); }
    .input-area textarea:focus { border-color: var(--lc-accent); }
    .send-btn {
      width: 36px;
      height: 36px;
      border-radius: 10px;
    ...
    .session-search:focus { border-color: var(--lc-accent); }
    .session-list {
      flex: 1;
      overflow-y: auto;
      padding: 8px 12px;
    ...
    .session-item:hover { background: rgba(255,255,255,0.04); border-color: var(--lc-border-soft); }
    .session-item-title { font-weight: 600; font-size: 13px; color: var(--lc-text); margin-bottom: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .session-item-preview { font-size: 11px; color: var(--lc-text-dim); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .session-item-time { font-size: 10px; color: var(--lc-text-soft); margin-top: 2px; }
    .session-empty { text-align: center; padding: 32px; color: var(--lc-text-soft); }
  </style>
</head>
<body>
  <div class="chat-header">
    <div class="brand">Lya<span class="brand-accent">Cloud</span></div>
  ...
  <div class="messages" id="messages">
    <div class="welcome" id="welcomeScreen">
      <div class="welcome-title">Lya<span class="accent">Cloud</span></div>
  ...
  <div class="input-area">
    <textarea id="chatInput" placeholder="Message Lya Cloud..." rows="1"></textarea>
