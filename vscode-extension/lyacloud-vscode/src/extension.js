...
const { ChatController, LyaCloudChatViewProvider, LyaCloudChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const LYACLOUD_REPO_URL = 'https://github.com/StudioCodeAI/lyacloud';
const LYACLOUD_SETUP_URL = 'https://github.com/StudioCodeAI/lyacloud/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.lyacloud-profile.json';
const SECRET_AZURE_API_KEY = 'lyacloud.azure.apiKey';
...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'Lya Cloud Azure chat is enabled but endpoint or deployment is missing. Run "Lya Cloud: Configure Azure / Foundry Chat" or set lyacloud.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'Lya Cloud Azure chat is enabled but no API key is set. Use "Lya Cloud: Set Azure / Foundry API Key" or lyacloud.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'Lya Cloud — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('Lya Cloud Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('Lya Cloud Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('lyacloud');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'Lya Cloud — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'Lya Cloud — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'Lya Cloud — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'Lya Cloud Azure / Foundry chat saved. Launch Lya Cloud from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('lyacloud');
  const launchCommand = configured.get('launchCommand', 'lyacloud');
  const terminalName = configured.get('terminalName', 'Lya Cloud');
...
async function launchLyaCloud(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Lya Cloud command not found: ${executable}. Install it with: npm install -g @studiocodeai/lyacloud@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(LYACLOUD_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(LYACLOUD_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Lya Cloud because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
...
<head>
  ...
  <style>
    :root {
      --lc-bg: #050505;
      --lc-panel: #110d0c;
      --lc-panel-strong: #17110f;
      --lc-panel-soft: #1d1512;
      --lc-border: #645041;
      --lc-border-soft: rgba(220, 195, 170, 0.14);
      --lc-text: #f7efe5;
      --lc-text-dim: #dcc3aa;
      --lc-text-soft: #aa9078;
      --lc-accent: #d77757;
      --lc-accent-bright: #f09464;
      --lc-accent-soft: rgba(240, 148, 100, 0.18);
      --lc-positive: #e8b86b;
      --lc-warning: #f3c969;
      --lc-critical: #ff8a6c;
      --lc-focus: #ffd3a1;
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Lya Cloud wordmark">Lya<span class="wordmark-accent">Cloud</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the Studio CodeAI Lya Cloud project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Lya Cloud commands quickly.</span>
            </button>
...
class LyaCloudControlCenterProvider {
  constructor() {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchLyaCloud();
          break;
        case 'launchRoot':
          await launchLyaCloud({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'lyacloud.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  const provider = new LyaCloudControlCenterProvider();
  ...
  const chatViewProvider = new LyaCloudChatViewProvider(chatController);
  const chatPanelManager = new LyaCloudChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) Lya Cloud';
  statusBarItem.tooltip = 'Open Lya Cloud Chat';
  statusBarItem.command = 'lyacloud.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Lya Cloud';
        statusBarItem.tooltip = 'Lya Cloud is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Lya Cloud';
        statusBarItem.tooltip = 'Lya Cloud connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Lya Cloud';
        statusBarItem.tooltip = 'Open Lya Cloud Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('lyacloud.start', async () => {
    await launchLyaCloud();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'lyacloud.startInWorkspaceRoot',
    async () => {
      await launchLyaCloud({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('lyacloud.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(LYACLOUD_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'lyacloud.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(LYACLOUD_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'lyacloud.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('lyacloud.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.lyacloud');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('lyacloud.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('lyacloud.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('lyacloud.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('lyacloud.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('lyacloud.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('lyacloud.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('lyacloud.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('lyacloud.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'lyacloud.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'lyacloud.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('lyacloud')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  LyaCloudControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  LyaCloudChatViewProvider,
  LyaCloudChatPanelManager,
};
