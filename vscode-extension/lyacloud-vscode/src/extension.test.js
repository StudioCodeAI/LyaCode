...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'lyacloud',
    launchCommand: 'lyacloud --project-aware',
    terminalName: 'Lya Cloud',
    shimEnabled: false,
    workspaceFolder: '/workspace/lyacloud/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/lyacloud/very/long/path/example-project',
    launchCwdLabel: '/workspace/lyacloud/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/lyacloud/very/long/path/example-project/.lyacloud-profile.json',
    workspaceProfilePath: '/workspace/lyacloud/very/long/path/example-project/.lyacloud-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Lya Cloud wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(html, /Lya<span class="wordmark-accent">Cloud<\/span>/);
  ...
  assert.match(
    html,
    /title="\/workspace\/lyacloud\/very\/long\/path\/example-project"[^>]*>\/workspace\/lyacloud\/very\/long\/path\/example-project<\//,
  );
});
...
test('LyaCloudControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { LyaCloudControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/lyacloud/src/panels/control-center.js',
      workspacePath: '/workspace/lyacloud',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/lyacloud/src/panels',
      projectAwareCwdLabel: '/workspace/lyacloud/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/lyacloud',
      workspaceRootCwdLabel: '/workspace/lyacloud',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/lyacloud',
      activeFilePath: '/workspace/lyacloud/src/panels/control-center.js',
      workspacePath: '/workspace/lyacloud',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/lyacloud',
      projectAwareCwdLabel: '/workspace/lyacloud',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/lyacloud/src/panels',
      launchCwdLabel: '/workspace/lyacloud/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/lyacloud\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/lyacloud/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/lyacloud',
      launchCwdLabel: '/workspace/lyacloud',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/lyacloud/);
  assert.match(html, /Same workspace-root target as Launch Lya Cloud because the relative command resolves from the workspace root · \/workspace\/lyacloud/);
});
