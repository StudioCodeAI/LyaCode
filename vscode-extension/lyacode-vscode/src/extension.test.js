...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'lyacode',
    launchCommand: 'lyacode --project-aware',
    terminalName: 'Lya Code',
    shimEnabled: false,
    workspaceFolder: '/workspace/lyacode/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/lyacode/very/long/path/example-project',
    launchCwdLabel: '/workspace/lyacode/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/lyacode/very/long/path/example-project/.lyacode-profile.json',
    workspaceProfilePath: '/workspace/lyacode/very/long/path/example-project/.lyacode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Lya Code wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/lyacode\/very\/long\/path\/example-project"[^>]*>\/workspace\/lyacode\/very\/long\/path\/example-project<\//,
  );
});
...
test('LyaCodeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { LyaCodeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/lyacode/src/panels/control-center.js',
      workspacePath: '/workspace/lyacode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/lyacode/src/panels',
      projectAwareCwdLabel: '/workspace/lyacode/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/lyacode',
      workspaceRootCwdLabel: '/workspace/lyacode',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/lyacode',
      activeFilePath: '/workspace/lyacode/src/panels/control-center.js',
      workspacePath: '/workspace/lyacode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/lyacode',
      projectAwareCwdLabel: '/workspace/lyacode',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/lyacode/src/panels',
      launchCwdLabel: '/workspace/lyacode/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/lyacode\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/lyacode/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/lyacode',
      launchCwdLabel: '/workspace/lyacode',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/lyacode/);
  assert.match(html, /Same workspace-root target as Launch Lya Code because the relative command resolves from the workspace root · \/workspace\/lyacode/);
});
