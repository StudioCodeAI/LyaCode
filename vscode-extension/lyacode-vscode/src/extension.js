...
const { ChatController, LyaCodeChatViewProvider, LyaCodeChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const LYACODE_REPO_URL = 'https://github.com/StudioCodeAI/lyacode';
const LYACODE_SETUP_URL = 'https://github.com/StudioCodeAI/lyacode/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.lyacode-profile.json';
const SECRET_AZURE_API_KEY = 'lyacode.azure.apiKey';
...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'Lya Code Azure chat is enabled but endpoint or deployment is missing. Run "Lya Code: Configure Azure / Foundry Chat" or set lyacode.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'Lya Code Azure chat is enabled but no API key is set. Use "Lya Code: Set Azure / Foundry API Key" or lyacode.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'Lya Code — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('Lya Code Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('Lya Code Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('lyacode');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'Lya Code — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'Lya Code — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'Lya Code — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'Lya Code Azure / Foundry chat saved. Launch Lya Code from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('lyacode');
  const launchCommand = configured.get('launchCommand', 'lyacode');
  const terminalName = configured.get('terminalName', 'Lya Code');
...
async function launchLyaCode(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Lya Code command not found: ${executable}. Install it with: npm install -g @studiocodeai/lyacode@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(LYACODE_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(LYACODE_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Lya Code because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Lya Code wordmark">Lya<span class="wordmark-accent">Cloud</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the Studio CodeAI Lya Code project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Lya Code commands quickly.</span>
            </button>
...
class LyaCodeControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchLyaCode({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'lyacode.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  const provider = new LyaCodeControlCenterProvider();
  ...
  const chatViewProvider = new LyaCodeChatViewProvider(chatController);
  const chatPanelManager = new LyaCodeChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) Lya Code';
  statusBarItem.tooltip = 'Open Lya Code Chat';
  statusBarItem.command = 'lyacode.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Lya Code';
        statusBarItem.tooltip = 'Lya Code is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Lya Code';
        statusBarItem.tooltip = 'Lya Code connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Lya Code';
        statusBarItem.tooltip = 'Open Lya Code Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('lyacode.start', async () => {
    await launchLyaCode();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'lyacode.startInWorkspaceRoot',
    async () => {
      await launchLyaCode({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('lyacode.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(LYACODE_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'lyacode.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(LYACODE_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'lyacode.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('lyacode.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.lyacode');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('lyacode.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('lyacode.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('lyacode.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('lyacode.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('lyacode.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('lyacode.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('lyacode.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('lyacode.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'lyacode.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'lyacode.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('lyacode')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  LyaCodeControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  LyaCodeChatViewProvider,
  LyaCodeChatPanelManager,
};
